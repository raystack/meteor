---
title: Context Graph for AI
description: How Meteor builds the structured knowledge layer that AI systems need to reason accurately.
order: 7
---
...
Meteor's extractors connect to 35 data sources — databases (BigQuery, Postgres, Snowflake), BI tools (Tableau, Metabase, Superset), streaming platforms (Kafka), cloud storage (GCS), orchestrators (Optimus), and more. Each extractor produces **Record**s — an **Entity** with flat properties plus **Edge**s representing relationships:
