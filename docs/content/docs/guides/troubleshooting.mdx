...
**Solutions:**
- If the data source uses a self-signed certificate, check the [extractor's docs page](../extractors/overview) for SSL configuration options.
...
- Check that the extractor and sink names match registered plugin names. Use `meteor plugins list --type extractor` and `meteor plugins list --type sink` to see available plugins.
...
- Run `meteor plugins list --type extractor` or `meteor plugins list --type sink` to confirm the plugin is registered.
