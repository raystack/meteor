...
Processors transform [Records](metadata_models.md) in-flight between extraction and sinking. Each processor receives a Record (Entity + Edges), modifies the Entity, and returns the updated Record. Edges pass through unchanged.

Processors are defined in the `processors` block of a [recipe](../concepts/recipe.md) and execute sequentially -- the output of one processor becomes the input of the next:
