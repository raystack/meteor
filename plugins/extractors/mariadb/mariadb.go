...
func (e *Extractor) processTable(ctx context.Context, database, tableName string) error {
	...
	tableURN := models.NewURN("mariadb", e.UrnScope, "table", fmt.Sprintf("%s.%s", database, tableName))
	entity := models.NewEntity(tableURN, "table", tableName, "mariadb", map[string]any{"columns": columns})
