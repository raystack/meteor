...
func buildProps(t *testing.T, m map[string]any) *structpb.Struct {
	t.Helper()
...
func replaceServerURL(cfg map[string]any, serverURL string) {
	reqCfg := cfg["request"].(map[string]any)
	reqCfg["url"] = strings.Replace(reqCfg["url"].(string), "{{serverURL}}", serverURL, 1)
	scriptCfg := cfg["script"].(map[string]any)
	scriptCfg["source"] = strings.Replace(scriptCfg["source"].(string), "{{serverURL}}", serverURL, -1)
	beforeScriptCfg, ok := cfg["before_script"].(map[string]any)
	if ok {
