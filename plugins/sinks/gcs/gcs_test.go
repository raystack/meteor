...
func TestInit(t *testing.T) {
	t.Run("should return error if config is invalid", func(t *testing.T) {
		...
		actualError := gcsSink.Init(context.TODO(), plugins.Config{RawConfig: map[string]any{
			"project_id": "",
		}})
...
func TestSink(t *testing.T) {
	t.Run("should write data in bucket and return nil error on success", func(t *testing.T) {
		entity := models.NewEntity("", "user", "", "", map[string]any{
			"full_name": "John Doe",
		...
		err := gcsSink.Init(context.TODO(), plugins.Config{RawConfig: map[string]any{
			"project_id":           "google-project-id",
			"url":                  "gcs://bucket_name/target_folder",
